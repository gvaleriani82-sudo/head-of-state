...
function goAppoint(){
  ...
  /* L64-4 — le liste ai competenti: la rosa parte da competenza 2 e sale di 1 (tetto 3) — un gruppo che sa fare alimenta le carte future */
  if(typeof S!=='undefined' && S && S.gruppo && S.gruppo.competenti>=50){ for(const m of MINISTRIES){ (APT.cands[m.id]||[]).forEach(function(c){ c.comp=Math.min(3, Math.max(2, (c.comp||1)+1)); }); } }
  /* L64-3 — lo sfidante promosso entra nella rosa (primo dicastero), competenza 3: chi ti fa ombra bene diventa il tuo problema */
  if(typeof S!=='undefined' && S && S.candidatoPromosso && MINISTRIES.length){ /* ⚠ alla prima nomina S non esiste ancora */ const m0=MINISTRIES[0].id, c0=Object.assign(mkCand(), S.candidatoPromosso); APT.cands[m0][0]=c0; S.log.unshift({t:T('Lo sfidante'), x:T('%V, promosso, entra nella rosa dei ministri.').replace('%V',c0.nm)}); S.candidatoPromosso=null; }
...
/* ================================================================================================================
   L101-1b · L'INFLAZIONE D'EPOCA. Il gioco non ha un indicatore d'inflazione (L28-1): `S.inflazione` è il seme della
   porta, fisso, e il suo solo lettore è l'erosione del debito (model.js, crescita nominale = reale + inflazione).
   Questa tabella gli dà un anno: **valori ASSOLUTI** (non scostamenti come il ciclo), ⚠ INSEE, ordine di grandezza.
   `inflazioneAnno()` legge la riga dell'era in vigore per `S.year`; senza riga (prima del 1970, le altre linee, il
   presente) torna `S.inflazione` com'è sempre stato — additiva: le porte senza riga sono identiche.
   ⚠ La tabella SCAVALCA il tetto del seme (`economia.inflazioneTetto`, 15 per la Francia '70 e '80), dichiaratamente:
   il 13,6 del 1980 è vero. Non c'è più un clamp 0-10: da L91-1 il tetto è del paese.
   ================================================================================================================ */
const DRIFT_INFLAZIONE_ERA = {
  [LINEA_FR]: [ {da:1970, inf:5.2}, {da:1973, inf:7.3}, {da:1974, inf:13.7}, {da:1975, inf:11.8}, {da:1976, inf:9.6},
                {da:1978, inf:9.1}, {da:1979, inf:10.8}, {da:1980, inf:13.6}, {da:1981, inf:13.4}, {da:1982, inf:11.8},
                {da:1983, inf:9.6}, {da:1984, inf:7.4}, {da:1985, inf:5.8}, {da:1986, inf:2.7}, {da:1987, inf:3.1},
...
function inflazioneAnno(){
  ...
  var W = S.era ? DRIFT_INFLAZIONE_ERA[S.era] : null, v = null;
  if(W) for(var i=0;i<W.length;i++){ if(S.year>=W[i].da) v=W[i].inf; }
  return (v!=null) ? v : (S.inflazione||0);
}
/* ================================================================================================================
   L102-2 · IL DEFICIT D'EPOCA. `S.deficitBase` è un seme fisso, il decennio vero no: col pavimento del debito a 5
   (L102-1) quattro porte scendevano SOTTO la storia (uk2000 33 → 8, uk1990 → 25, italia1970 → 30, uk1970 → 30),
   perché nulla diceva «qui lo Stato spende di più». Stessa forma della disoccupazione: **scostamenti per anno sul
   `deficitBase`**, positivi = più disavanzo (la convenzione di `computeDeficit`), letti da `deficitEra()` e SOMMATI
   alla base in model.js. Assente (le altre linee, gli anni senza riga) = 0: additiva.
   ⚠ La calibrazione dell'avvio (`deficitTarget`, startGame) assorbe il valore in vigore NEL MESE D'AVVIO: la riga
   dell'anno d'avvio di una porta non sposta il disavanzo di partenza, solo le successive (per questo ogni decennio
   comincia da una riga 0). E una riga vale finché non ne arriva un'altra: le righe `{da:1980, def:0}` chiudono il
   '70 perché non entri nella porta del decennio dopo.
   ⚑ I valori sono CERCATI con la sweep (.claude/misura-deficit-era.js), non scritti a occhio: il bersaglio è il
   DEBITO reso, non il deficit. */
const DRIFT_DEFICIT_ERA = {
  /* ⚠ LE RIGHE INGLESI SONO GRANDI (fino a +10,5) e non perché il disavanzo inglese lo fosse: sulla linea UK il
  ...
  /* L103-1 · fr1990: 35 nel 1990 → 45 nel '93 → 57 nel '96 → 58 nel '99 → 57 nel 2001, il decennio in cui il debito francese
     ...
     (.claude/misura-fr1990-struttura.js sweep def, PS, 5 semi) rende 35,6 · 46,4 ('93) · 56,2 ('96) · 57,9 ('99) · 56,9 (2001),
...
function deficitEra(){
  ...
  var W = DRIFT_DEFICIT_ERA[S.era];
  if(!W || !W.length) return 0;
  var v=0;
  for(var i=0;i<W.length;i++){ if(S.year>=W[i].da) v=W[i].def; }
  return v;
}
...
function initStatoBase(){
  ...
  S.falkland=null; S.minatori80=null; S.polltax=null; S.alleanza=null;   // L60-2 (dati puri, round-trip)
  ...
  S.gruppo=null; S.sfidaSeme=null;   // L64-4 (dati puri)
  S.scioglimentiChiesti=0; S.scioglimentoScelto=false;   // L56-1 — dati puri, round-trip
  S.legislaturaDa=null;   // L104-2: il mese (anno×12+mese) in cui è nata l'Assemblea in vigore nel semipresidenziale (tappa o scioglimento); null = con la presidenza
...
/* ============================================================
   ...
   ============================================================ */
/* L104-3 · INSTALLARE UN PROFILO DI LEVE NON CAMBIA IL DISAVANZO NEL MESE IN CUI SUCCEDE. `initStatoBase` calibra
   `S.deficitBase` perché il disavanzo d'avvio sia quello del seme, con le leve di quel momento (neutre). Chi sostituisce
   `S.pol` per intero dopo — l'avversario che si insedia (entraOpposizione: all'avvio da sfidante e dopo una sconfitta),
   il premier-AI del ministro, i percorsi «dormienti» a profilo centro — si portava dietro il costo del suo profilo senza
   che nessuno ricalibrasse: +9,1 punti di disavanzo con un governo di sinistra, +3,3 di destra (L104-1). Qui la base si
   riallinea perché `computeDeficit()` dia lo stesso numero prima e dopo lo scambio. Le leve mosse DAL GIOCATORE
   (`setPol`) non passano di qui: quelle devono pesare. È l'UNICO punto che scrive `S.pol` per intero. */
function installaProfilo(pol){
  var prima=(S && S.ind && typeof computeDeficit==='function' && S.pol) ? computeDeficit() : null;
  S.pol=Object.assign({}, pol);
  if(prima!=null){ var dopo=computeDeficit(); S.deficitBase=(S.deficitBase!=null?S.deficitBase:3.0)+(prima-dopo); }
}
...
function startMinistro(){
  ...
  installaProfilo(GOVERNI_PROFILI[profByAsse(S.premier.asse)]);   // la politica nazionale è del premier (tranne il tuo dicastero, che muovi tu) — L104-3: senza salto di disavanzo
  S.snap=Object.assign({},S.pol);
...
function diventaPremier(viaElezione){
  ...
  S.seggi=(sistemaSemipres() && S.seggi) ? S.seggi : ((PAESE.coalizione||PAESE.comeSiVince==='parlamentare')?calcSeggi():null);   // L104-2: nel semipresidenziale la salita eredita l'Assemblea
  ...
  if(typeof aggiornaCoabitazione==='function') aggiornaCoabitazione();   // L100-2/L104-2 (dopo il reset di mesiMinoranza): alla salita si deriva
...
function startDiplomatico(){
  ...
  installaProfilo(GOVERNI_PROFILI['centro']); S.snap=Object.assign({},S.pol);   // nazionale neutro/dormiente (L104-3: dal punto unico)
...
function startLocale(){
  ...
  S.log=[{t:'Eletto '+ruolo, x:luoghiSub(T('Cominci dal basso: amministra bene %L, costruisci la notorietà, e il partito ti chiamerà %ACAPITALE.')).replace('%L',S.locale.nome)}];
...
function eraCartaViva(E, def){
  /* L79-1 - IL GANCIO-PAESE. Una carta che dichiara `paesi:[...]` vive SOLO in quei paesi. Sta qui, in cima al
     passaggio unico dell'era, perche' questo e' il punto da cui passano tutti i pool: verificato, non dedotto —
     EVENTS (game.js:3924), DOSSIERS (502 e 4010), BEAT_LEGGERI (2204) e TITOLI (2738) filtrano tutti per
     eraViva/eraVivaT, e in tutto i punti che filtrano per era sono 46.
     E' ADDITIVO: una carta senza `paesi` passa esattamente come prima, quindi nessuna regressione.
     Si DICHIARA, non si simula con `cond`: un `cond` che guarda S.paese e' opaco alle guardie e ai censimenti. */
  if(E && E.paesi){
...
const DRIFT_ECONOMICO_ERA = {
  ...
  [LINEA_UK]: [ {da:1950, ciclo:18.4}, {da:1951, ciclo:3},  {da:1953, ciclo:20},
                ...
                /* ==========================================================================================
                   L60-2 · IL DECENNIO '80 — la cura e il suo prezzo (scheda §2). Il drift NOMINALE torna a
                   salire: 1980-81 la stretta (recessione vera, il fondo), dall'82 la ripresa, dall'86 il boom
                   della City e le privatizzazioni. Il debito, che nel '70 saliva, qui smette e SCENDE nella
                   seconda metà — è quello che la scheda chiede. L'inflazione piegata (18% → 4-5%) resta, come
                   sempre su questa linea, una cosa che i testi dicono e il motore non mostra (L28-1).
                   La disoccupazione, il numero del decennio, ha una tabella sua: DRIFT_DISOCCUPAZIONE_ERA.
                   ========================================================================================== */
                /* ⚑ TARATURA (misurata, stessa scelta fra due beni di L58-1): con [3 6 9 13 15] la crescita stava
                   inchiodata al clamp +5 dal 1982 al 1989 e la recessione dell'80-81 non si vedeva (3,9). Scelta
                   la leggibilità: 1980-81 sotto zero (la stretta), poi una salita che tocca il clamp dal 1985.
                   Misurato (seme 11, scelte 0): crescita −0,2/−0,5 nell'80-81, 2,9-3,0 nell'82-83, 4,8 nell'84, poi 5;
                   disoccupazione 8,7 → 10,8 (1983) → 4,7 (1989); debito 47 → 77 (1985) → giù nella seconda metà. */
                ...
                /* ⚑ TARATURA misurata (stessa scelta fra due beni di L58-1 e L60-2): con ciclo 7 la crescita restava
  ...
  [LINEA_FR]: [ {da:1950, ciclo:4},   {da:1951, ciclo:6},   {da:1953, ciclo:2},
                ...
                /* L101-1 · GLI ANNI '80 (scheda PRESET-FRANCIA-1980 §2), scostamenti sulla crescita-base 2,2. Sweep a tre profili
                   (.claude/misura-fr1980-struttura.js ciclo, crescita resa a luglio, RPR, 5 semi): A (la scheda) tiene l'83 a 1,7
                   — la stretta non si sente — e l'88-89 a 3,6/3,9, sotto il 4 chiesto; **B (adottato)** l'83-84 a 0,8, l'88-89
                   a 4,5/4,6; C alza anche l'86-87 (2,4/2,5) per un guadagno di due decimi sull'88-89. */
                {da:1980, ciclo:-0.5},{da:1981, ciclo:-1},  {da:1982, ciclo:0},   {da:1983, ciclo:-2},  {da:1984, ciclo:-1},
                {da:1986, ciclo:0},   {da:1988, ciclo:2.5}, {da:1989, ciclo:2.5},
                /* L103-1 · GLI ANNI '90 (scheda PRESET-FRANCIA-1990 §2), scostamenti sulla crescita-base 2,0. Le righe 1990-91
                   dell'80 (+0,5 · −1) sono sostituite: stavano oltre i 120 mesi di fr1980 e non ne toccano le misure.
                   Discesa per coordinate (.claude/misura-fr1990-struttura.js sweep ciclo, PS, 5 semi, resa a luglio): il profilo
                   della scheda lasciava il '93 a −0,1 (la recessione non si vedeva) e il '98 a 3,1; adottato: 90 3,0 · 91 0,9 ·
...
const DRIFT_DISOCCUPAZIONE_ERA = {
  ...
  [LINEA_FR]: [ {da:1950, un:-5}, {da:1967, un:-4.5}, {da:1969, un:-4},
                ...
                /* L101-1 · gli anni '80: la disoccupazione a due cifre (scheda §2: 6,3 nel 1980 → 10 nel '85 → 10,5 nel '87 → 9,5).
                   CERCATI con la sweep (.claude/misura-fr1980-struttura.js disocc, discesa per coordinate a passi di 0,5, RPR,
                   5 semi, resa a luglio): 80 6,4 · 83 8,4 · 85 10,1 · 87 10,5 · 89 9,6 — tutti dentro ±0,5. Il 1991 è fuori dai
                   120 mesi misurati. */
                {da:1980, un:-1.5}, {da:1982, un:0}, {da:1984, un:1.5}, {da:1986, un:2.5}, {da:1988, un:3.5},
                /* L103-1 · gli anni '90 (scheda PRESET-FRANCIA-1990 §2: 8,9 nel 1990 → 12,6 nel '94 → 12,3 nel '97 → 9,1 nel 2000).
                   CERCATE con la sweep (.claude/misura-fr1990-struttura.js sweep un, discesa per coordinate a passi 1 · 0,5 · 0,25,
...
function disoccupazioneEra(){
  if(typeof S==='undefined' || !S) return 0;
  var W = DRIFT_DISOCCUPAZIONE_ERA[S.era || 'contemporanea'];
  if(!W || !W.length) return 0;
  var y=S.year, base=0;
  for(var i=0;i<W.length;i++){ if(y>=W[i].da) base=W[i].un; }
  return base;
}
...
/* L103-1 · i Verdi che entrano alla tappa del 1997/6 o del 1998/3 (una sola scatta): un oggetto solo, così le due tappe
   gemelle non possono divergere. `selezionabile:false` (D25: sette deputati, una carriera che non si può giocare). */
const VERDI_1997 = { id:'fr_verts', nome:'Verdi', orientamento:'ecologista', base:{ giovani:0.5, cetomedio:0.5 }, forza:7.9, asse:-1,
                     alleati:['fr_sfio','fr_pcf'], selezionabile:false, nota:'I Verdi del 1997 hanno sette deputati e governano col PS: non è una carriera che si possa giocare' };
const RIALLINEAMENTI_ERA = {
  ...
  [LINEA_UK]: {
    ...
    /* L60-2 · IL DECENNIO '80 (scheda PRESET-UK-1980 §1, ukpolitical.info). Il roster resta a CINQUE: la scissione
       dell'81 è `rinomina uk_lib → Alleanza SDP-Liberali` + `delta` dal Labour (correzione del disegno, 1/8): l'SDP
       non è un'entità di roster, confluì subito. `se`: chi gioca Labour o Liberal la vive come SNODO, non come tappa.
       1983 (giu): Con 42,4 (397) · Lab 27,6 (209) · Alleanza 25,4 (23) — a 1,6 punti dal Labour e 186 seggi in
             meno: il caso più famoso del maggioritario, dichiarato come dato (L61). I delta partono dal dopo-'81.
       1987 (giu): Con 42,2 (376) · Lab 30,8 (229) · Alleanza 22,6 (22) — terzo mandato di fila.
       1988 (mar): l'Alleanza si fonde e diventa Liberal Democratici (rinomina; non per chi gioca uk_lib: L40-1).
       Seggi su 633 (senza l'Irlanda del Nord, come le tappe del '74). */
    '1981/3': { se:function(){ return S.partito!=='uk_lab' && S.partito!=='uk_lib'; },
            rinomina:[ {id:'uk_lib', nome:'Alleanza SDP-Liberali'} ],
            delta:[ {id:'uk_lab',delta:-7}, {id:'uk_lib',delta:7} ] },
    '1983/6': { delta:[ {id:'uk_con',delta:-1.5}, {id:'uk_lab',delta:-2.3}, {id:'uk_lib',delta:4.6}, {id:'uk_snp',delta:-0.5} ],
            urne:  { uk_con:42.4, uk_lab:27.6, uk_lib:25.4, uk_snp:1.1, uk_plaid:0.4 },
            seggi: { uk_con:62.72, uk_lab:33.02, uk_lib:3.63, uk_snp:0.32, uk_plaid:0.32 } },   // 397/209/23/2/2 su 633 · maggioranza 144
    '1987/6': { delta:[ {id:'uk_con',delta:-0.2}, {id:'uk_lab',delta:3.2}, {id:'uk_lib',delta:-2.8}, {id:'uk_snp',delta:0.2}, {id:'uk_plaid',delta:-0.1} ],
            urne:  { uk_con:42.2, uk_lab:30.8, uk_lib:22.6, uk_snp:1.3, uk_plaid:0.3 },
            seggi: { uk_con:59.40, uk_lab:36.18, uk_lib:3.48, uk_snp:0.47, uk_plaid:0.47 } },   // 376/229/22/3/3 su 633 · maggioranza 102
    '1988/3': { se:function(){ return S.partito!=='uk_lib'; },
            rinomina:[ {id:'uk_lib', nome:'Liberal Democratici'} ] },
    /* L75-1 · IL DECENNIO '90 (scheda PRESET-UK-1990 §1). 1992 (apr): Con 41,9 (336) · Lab 34,4 (271) · LibDem 17,8 (20) —
       contro i sondaggi: nei sei mesi prima i sondaggi resi al giocatore hanno un BIAS (SONDAGGI_BIAS) di −6 per un
       blocco di destra e +6 per uno di sinistra, e le urne dichiarate dicono il contrario. 1997 (mag): Lab 43,2 (418) ·
       Con 30,7 (165) · LibDem 16,8 (46), maggioranza 179, la frana. Seggi su 634 e 641 (senza l'Irlanda del Nord). */
    '1992/4': { delta:[ {id:'uk_con',delta:-0.3}, {id:'uk_lab',delta:3.6}, {id:'uk_lib',delta:-4.8}, {id:'uk_snp',delta:0.6}, {id:'uk_plaid',delta:0.2} ],
            urne:  { uk_con:41.9, uk_lab:34.4, uk_lib:17.8, uk_snp:1.9, uk_plaid:0.5 },
            seggi: { uk_con:53.0, uk_lab:42.7, uk_lib:3.2, uk_snp:0.5, uk_plaid:0.6 } },   // 336/271/20/3/4 su 634 · maggioranza 21
    '1997/5': { delta:[ {id:'uk_con',delta:-11.2}, {id:'uk_lab',delta:8.8}, {id:'uk_lib',delta:-1.0}, {id:'uk_snp',delta:0.1} ],
            urne:  { uk_con:30.7, uk_lab:43.2, uk_lib:16.8, uk_snp:2.0, uk_plaid:0.5 },
            seggi: { uk_lab:65.2, uk_con:26.1, uk_lib:7.2, uk_snp:0.9, uk_plaid:0.6 } },   // 418/165/46/6/4 su 641 · maggioranza 179
    /* L77-1 · IL DECENNIO 2000 (scheda PRESET-UK-2000 §1, ukpolitical.info). Seggi su 639/625/631 (senza l'Irlanda
       del Nord, come tutte le tappe inglesi). 2001: la vittoria piu' larga e la meno partecipata (affluenza 59,4%).
       2005: maggioranza di 68 col 35,2% dei voti — il caso piu' estremo di maggioranza fabbricata della linea, e i
       seggi sono DICHIARATI: il modello non viene chiamato. 2010: PARLAMENTO APPESO (nessuno arriva a 50), il
       secondo dal 1929, e i Verdi entrano nel roster col loro primo seggio (G12: l'UKIP resta fuori, e' un evento). */
    '2001/6': { delta:[ {id:'uk_lab',delta:-2.5}, {id:'uk_con',delta:1.0}, {id:'uk_lib',delta:1.5}, {id:'uk_snp',delta:-0.2}, {id:'uk_plaid',delta:0.2} ],
            urne:  { uk_lab:40.7, uk_con:31.7, uk_lib:18.3, uk_snp:1.8, uk_plaid:0.7 },
            seggi: { uk_lab:64.47, uk_con:25.98, uk_lib:8.14, uk_snp:0.78, uk_plaid:0.63 } },   // 412/166/52/5/4 su 639 · maggioranza 166
    '2005/5': { delta:[ {id:'uk_lab',delta:-5.5}, {id:'uk_con',delta:0.7}, {id:'uk_lib',delta:3.7}, {id:'uk_snp',delta:-0.3}, {id:'uk_plaid',delta:-0.1} ],
            urne:  { uk_lab:35.2, uk_con:32.4, uk_lib:22.0, uk_snp:1.5, uk_plaid:0.6 },
            seggi: { uk_lab:56.96, uk_con:31.68, uk_lib:9.92, uk_snp:0.96, uk_plaid:0.48 } },   // 356/198/62/6/3 su 625 · maggioranza 68 col 35,2%
    '2010/5': { entra:[ { id:'uk_green', nome:'Verdi', orientamento:'sinistra', base:{ giovani:0.6, cetomedio:0.4 },
                          forza:1.0, asse:-1, gruppoUE:'verdi', terreno:-0.9, concentrazione:60 } ],
  ...
  [LINEA_FR]: {
    ...
    /* ==========================================================================================================
       L101-1 · LE TRE TAPPE DEGLI ANNI '80 (scheda PRESET-FRANCIA-1980 §1). Seggi a 100 (resti più grandi, i non
       iscritti ripartiti secondo la tabella destra-sinistra del sito); delta = urne storiche rinormalizzate sul roster
       meno quelle della tappa prima (il metodo del '70). Σ dei delta = 0, salvo il 1986 dove fa −10,6 = la quota
       d'ingresso del Front national. Voti del 1° turno: france-politique.fr (legislative 1981/1986/1988, consultate
       il 24/9/2026); i radicali di sinistra (MRG) contati nel PS, come nel '73.
       ⚑ D17: il 1981 e il 1988 sono CONDIZIONATI sull'Eliseo di sinistra (`eliseoDiSinistra()`, qui sotto): sono le
       Assemblee sciolte dal Presidente appena eletto, come il 1968 dal maggio (D9). Il 1986 è incondizionato: ci arriva
       qualunque Presidente, e per uno di sinistra è la prima coabitazione (L100-2: si entra solo a una tappa).
       ========================================================================================================== */
    /* 1981: la sinistra all'Eliseo scioglie l'Assemblea e il PS ha la maggioranza assoluta da solo. */
    '1981/6': { se:function(){ return typeof eliseoDiSinistra==='function' && eliseoDiSinistra(); },
                delta:[ {id:'fr_sfio',delta:11.6}, {id:'fr_pcf',delta:-6.2}, {id:'fr_unr',delta:-3.3}, {id:'fr_ri',delta:-2.1} ],   // Σ=0
                urne:  { fr_sfio:37.4, fr_unr:20.9, fr_ri:19.2, fr_pcf:16.1 },
                seggi: { fr_sfio:58.9, fr_pcf:9.0, fr_unr:18.9, fr_ri:13.2 } },
    /* 1986: la proporzionale di una volta sola. Entra il Front national: `alleati:[]` (con nessuno) e
       `selezionabile:false` (D18: 35 seggi e nessuna alleanza possibile, sarebbe una carriera che non si può giocare).
       ⚠ Le liste comuni RPR-UDF (21,4) sono divise in proporzione alle liste separate (11,2 : 8,3) — inferenza dichiarata.
       ⚠ La forza d'ingresso è 10,6: il 9,7 delle urne rinormalizzato sul roster, come ogni altra forza. */
    '1986/3': { entra:[ { id:'fr_fn', nome:'Front national', orientamento:'destra populista',
                          base:{ lavoratori:0.4, cetomedio:0.3, pensionati:0.3 }, forza:10.6, asse:2, alleati:[],
                          selezionabile:false, nota:'Il Front national del 1986 ha trentacinque deputati e nessun alleato: non è una carriera che si possa giocare' } ],
                delta:[ {id:'fr_sfio',delta:-6.1}, {id:'fr_pcf',delta:-6.5}, {id:'fr_unr',delta:3.4}, {id:'fr_ri',delta:-1.4} ],   // Σ=−10,6 = l'ingresso del FN
                urne:  { fr_sfio:31.0, fr_unr:23.5, fr_ri:17.4, fr_pcf:9.8, fr_fn:9.7 },
                seggi: { fr_sfio:37.4, fr_pcf:6.3, fr_unr:27.8, fr_ri:22.4, fr_fn:6.1 } },
    /* 1988: il Presidente rieletto scioglie, torna il maggioritario, la sinistra ha la maggioranza relativa (e il
       blocco PS+PCF sopra 50: la coabitazione si spegne qui). Il FN resta con un seggio.
       ⚠ SEGGI RICALCOLATI: quelli della scheda sommavano 98,4. Col metodo della scheda stessa (UDC dentro l'UDF, i 7 non
       iscritti di destra pro-quota fra RPR e UDF, i 6 di sinistra fra PS e PCF, il FN a 1) su 575 seggi assegnati. */
    '1988/6': { se:function(){ return typeof eliseoDiSinistra==='function' && eliseoDiSinistra(); },
                delta:[ {id:'fr_sfio',delta:4.1}, {id:'fr_pcf',delta:1.2}, {id:'fr_unr',delta:-5.4}, {id:'fr_ri',delta:0.5}, {id:'fr_fn',delta:-0.4} ],   // Σ=0
                urne:  { fr_sfio:35.9, fr_unr:19.2, fr_ri:18.5, fr_pcf:11.3, fr_fn:9.7 },
                seggi: { fr_sfio:48.8, fr_pcf:4.4, fr_unr:23.2, fr_ri:23.4, fr_fn:0.2 } },
    /* ==========================================================================================================
       L103-1 · LE TRE TAPPE DEGLI ANNI '90 (scheda PRESET-FRANCIA-1990 §1). Seggi a 100 dalla scheda (gruppi AN, RL e NI
       pro-quota). Delta = urne del 1° turno rinormalizzate sul roster (resti più grandi) meno la tappa prima — per il 1993
       le forze d'avvio di fr1990 (le urne del 1988). Voti: france-politique.fr (legislative 1993 e 1997, consultate il
       25/9/2026); i radicali di sinistra contati nel PS, come nel '73 e nell''81; divers e altri fuori roster.
       ⚑ D24: il 1993 è incondizionato (la legislatura del 1988 scade per chiunque; per un Presidente di sinistra è la
       seconda coabitazione). Il 1997 e il 1998 sono LA COPPIA di S4 «Sciogliere» (L103-2): stessi seggi, stesso ingresso
       dei Verdi, condizioni che si escludono — il 1997 è l'Assemblea sciolta dal Presidente di destra, il 1998 la stessa
       Assemblea un anno dopo, per chi ha aspettato la scadenza o per l'Eliseo di sinistra che non scioglie.
       `S.scioglimento97`: null = storico (chi non gioca S4, l'opposizione), 'no' = «aspetta». Lo scrive S4 in L103-2.
       ========================================================================================================== */
    /* 1993: la disfatta della sinistra. RPR 20,2 · UDF 18,8 · PS+MRG 18,4 · PCF 9,3 · FN 12,5 (Verdi e GE fuori roster). */
    '1993/3': { delta:[ {id:'fr_unr',delta:5.2}, {id:'fr_ri',delta:4.2}, {id:'fr_sfio',delta:-14.7}, {id:'fr_pcf',delta:-0.2}, {id:'fr_fn',delta:5.5} ],   // Σ=0
                urne:  { fr_unr:20.2, fr_ri:18.8, fr_sfio:18.4, fr_fn:12.5, fr_pcf:9.3 },
                seggi: { fr_unr:45.6, fr_ri:38.4, fr_sfio:12.0, fr_pcf:4.0, fr_fn:0 } },
    /* 1997: il Presidente di destra scioglie con un anno d'anticipo e perde. Entrano i Verdi: forza 7,9 = il 6,8 delle urne
       rinormalizzato sul roster, come ogni altra forza (⚠ la scheda scriveva «≈ 6,8 rinormalizzata»: 6,8 è il voto,
       rinormalizzato è 7,9 — stesso metodo del FN nel 1986). `alleati:['fr_sfio','fr_pcf']` dal L104-1 (prima solo il PS, come diceva la scheda): la cricca avversa
       del '97 torna a tre, PS+PCF+Verdi = 56,6 — il PCF li nomina a sua volta (data.js, roster fr1990).
       PS+PRS 25,0 · RPR 15,7 · UDF 14,2 · PCF 9,9 · FN 14,9 · Verdi ed ecologisti 6,8 → 28,9 · 18,1 · 16,4 · 11,5 · 17,2 · 7,9. */
    '1997/6': { se:function(){ return typeof S!=='undefined' && S && typeof eliseoDiSinistra==='function' && !eliseoDiSinistra() && S.scioglimento97!=='no'; },
                entra:[ VERDI_1997 ],
    ...
    /* 1998: la gemella. La legislatura del '93 scade a marzo e la sinistra la vince come vinse il 1997: stessi seggi, stesso
       ingresso (dichiarato: non inventiamo un'altra Assemblea). Per l'Eliseo di sinistra spegne la coabitazione del '93. */
    '1998/3': { se:function(){ return typeof S!=='undefined' && S && typeof eliseoDiSinistra==='function' && (eliseoDiSinistra() || S.scioglimento97==='no'); },
                entra:[ VERDI_1997 ],
                delta:[ {id:'fr_sfio',delta:5.7}, {id:'fr_pcf',delta:-0.2}, {id:'fr_unr',delta:-7.4}, {id:'fr_ri',delta:-7.4}, {id:'fr_fn',delta:1.4} ],   // Σ=−7,9 = l'ingresso dei Verdi
                urne:  { fr_sfio:25.0, fr_unr:15.7, fr_fn:14.9, fr_ri:14.2, fr_pcf:9.9, fr_verts:6.8 },
                seggi: { fr_sfio:49.2, fr_pcf:6.2, fr_verts:1.2, fr_unr:24.6, fr_ri:19.8, fr_fn:0.2 } }
...
/* L101-1 · D17 · DI CHI È L'ELISEO. Una funzione sola, letta dalle tappe condizionate del 1981 e del 1988: la
   ...
   all'opposizione è `S.governoAvversario`, il partito che ha vinto l'urna contro di te (entraOpposizione, L98-2).
   «Di sinistra» = asse negativo. */
function eliseoDiSinistra(){
  if(typeof S==='undefined' || !S) return false;
...
/* L75-1 · I SONDAGGI CHE SBAGLIANO. La proiezione di L59-4 è onesta per costruzione (legge le forze); il 1992 inglese è
   ...
   giocatore porta un bias (contro un blocco di destra, a favore di uno di sinistra): la verità delle urne resta quella
   dichiarata alla tappa. Prevedibile e visibile è informazione; qui è la storia a essere stata ingannevole. */
const SONDAGGI_BIAS = { [LINEA_UK]: [ { da:1991*12+7, a:1992*12+4, bias:8 } ] };   // la stagione del voto di gioco (lug '91-gen '92) e i mesi fino alla tappa
function biasSondaggio(){
  if(typeof S==='undefined'||!S) return 0; const W=SONDAGGI_BIAS[S.era]; if(!W) return 0;
  const m=S.year*12+S.month; const w=W.find(function(x){ return m>=x.da && m<=x.a; }); if(!w) return 0;
  const a=(typeof asseBlocco==='function')?asseBlocco():0; return a>0 ? -w.bias : (a<0 ? w.bias : 0);
}
/* ============================================================================================================
...
function applicaRosterDelta(ricalcolaSeggi){
  ...
  (R.entra||[]).forEach(function(n){
    ...
    var np={ id:n.id, nome:n.nome, orientamento:n.orientamento||'centro', base:n.base||{}, forza:n.forza||1, asse:(n.asse!=null?n.asse:0), gruppoUE:n.gruppoUE||'noniscritti' };
    /* L101-1 · i campi dichiarati del partito che nasce NON si perdono più: `alleati` (con la lista vuota = «con nessuno»,
       il Front national del 1986), `selezionabile` e `nota` (D18). Prima la ricostruzione li scartava in silenzio — anche
       gli `alleati` dei Poujadisti del 1956 e dei Repubblicani indipendenti del 1962, che dopo la tappa tornavano all'asse. */
    if(Array.isArray(n.alleati)) np.alleati=n.alleati.slice();
    if(n.selezionabile===false) np.selezionabile=false;
    if(n.nota) np.nota=n.nota;
    lista.push(np);
  });
...
function riallineamentoTappa(){
  ...
  if(entry.seggi && S.seggi){
    ...
    if(atteso!=null && scarto > TAPPA_SOGLIA_SEGGI){
      ...
      if(typeof aggiornaCoabitazione==='function') aggiornaCoabitazione();   // L100-2/L104-2: la tappa è una legislativa: la coabitazione si deriva
      if(typeof sistemaSemipres==='function' && sistemaSemipres()) S.legislaturaDa=S.year*12+S.month;   // L104-2: l'Assemblea nuova nasce qui
    }
  }
  if(S.log){   // beat visibile della tappa (il quadro politico che si assesta)
    if(S.era===LINEA_IT && S.year===1958) S.log.unshift({t:T('Elezioni 1958'), x:T('Il quadro si assesta: il PSI cresce e si stacca, i monarchici arretrano.')});
    else if(S.era===LINEA_IT && S.year===1963) S.log.unshift({t:T('Elezioni 1963'), x:(S.apertura==='apri') ? T('Con l\'apertura a sinistra il PLI raddoppia: la reazione moderata prende forma.') : T('Il centrismo tiene: nessuna svolta, il logorio prosegue.')});
    else if(S.era===LINEA_IT && S.year===1972) S.log.unshift({t:T('Elezioni 1972'), x:T('La destra si unisce in un solo cartello e i monarchici vi confluiscono: il quadro si semplifica a destra.')});
    else if(S.era===LINEA_IT && S.year===1976) S.log.unshift({t:T('Elezioni 1976'), x:T('Il paese si divide in due grandi blocchi: i partiti laici minori vengono schiacciati.')});
    ...
    else if(S.era===LINEA_IT && S.year===1983) S.log.unshift({t:T('Elezioni 1983'), x:T('Il partito di maggioranza relativa tocca il suo minimo: i laici crescono e i socialisti diventano l\'ago della bilancia.')});
    else if(S.era===LINEA_IT && S.year===1987) S.log.unshift({t:T('Elezioni 1987'), x:T('I socialisti toccano il massimo dal dopoguerra, la sinistra storica arretra ancora, l\'onda laica si ritira.')});
    ...
    else if(S.era===LINEA_IT && S.year===1991) S.log.unshift({t:T('Il congresso della svolta'), x:T('Il partito comunista cambia nome e simbolo; una minoranza non segue e fonda la sua casa. La sinistra italiana si divide in due.')});
    else if(S.era===LINEA_IT && S.year===1992) S.log.unshift({t:T('Elezioni 1992'), x:T('Il partito di maggioranza relativa scende sotto una soglia mai toccata, e dal Nord entra in Parlamento una forza nuova. Il quadro non è più quello di sempre.')});
    else if(S.era===LINEA_IT && S.year===1994) S.log.unshift({t:T('La frana'), x:T('In un anno il partito che ha governato per mezzo secolo si scioglie e i socialisti chiudono; nasce un partito nuovo che al debutto è già primo, e la destra cambia nome. Il sistema dei partiti non esiste più.')});
    else if(S.era===LINEA_IT && S.year===1996) S.log.unshift({t:T('Elezioni 1996'), x:T('Due coalizioni si contendono il paese: il bipolarismo prende la forma che avrà per vent’anni.')});
    /* L44-1 — i beat dell'ultimo decennio */
    else if(S.era===LINEA_IT && S.year===2001) S.log.unshift({t:T('Elezioni 2001'), x:T('L\'alternanza è compiuta: si cambia governo con le urne, e nessuno lo trova più straordinario.')});
    else if(S.era===LINEA_IT && S.year===2006) S.log.unshift({t:T('Elezioni 2006'), x:T('Il paese si spacca a metà: alla Camera decidono poche migliaia di voti. Governare con quel margine sarà un\'altra cosa.')});
    ...
    /* L77-1 — le tre del 2000: l'affluenza che crolla, la maggioranza col 35%, e l'appeso che porta alla coalizione. */
    ...
    else if(S.era===LINEA_FR && S.year===1981) S.log.unshift({t:T('Elezioni del giugno 1981'), x:T('Il nuovo Presidente scioglie l\'Assemblea e la sinistra vince: il PS, da solo, ha più della metà dei seggi.')});   // L101-1
    else if(S.era===LINEA_FR && S.year===1986) S.log.unshift({t:T('Elezioni del marzo 1986'), x:T('Con la proporzionale di una volta sola la destra torna maggioranza per pochi seggi, e il Front national entra in aula.')});   // L101-1
    else if(S.era===LINEA_FR && S.year===1988) S.log.unshift({t:T('Elezioni del giugno 1988'), x:T('Il Presidente rieletto scioglie l\'Assemblea: la sinistra torna la più forte, con la maggioranza relativa.')});   // L101-1
  ...
  /* ⚑ L77-1 — OGNI NOTA-TAPPA E' FILTRATA PER LINEA. Tredici note italiane erano scritte if(S.year===N) senza dire
     di quale paese parlassero: la catena else-if si ferma alla prima che risponde, quindi sulle porte inglesi le
     tappe del 1983, 1987, 1992 e 2001 mostravano il testo ITALIANO (Craxi, la frana, l'alternanza). Tre di quelle
     collisioni erano gia' in piedi dai lotti UK precedenti e non le aveva viste nessuno; il 1979 era invece gia'
     filtrato, segno che il difetto era stato incontrato una volta e curato solo dove faceva male. Ora la guardia
     .claude/verifica-paese.js le cerca nel sorgente e diventa rossa se una nota torna senza linea. */
...
/* Chi paga l'economia sotto coabitazione: il termine economico di targetGroup e il logorio si moltiplicano per
   questi due numeri. ⚑ TARATI in L101-1 (D19) sulla porta fr1980, con .claude/misura-fr1980-struttura.js d19: consenso a dicembre
   1987 del Presidente PS in coabitazione contro un Presidente di destra al governo (RPR+UDF, 54,9, 40 carriere), banda
   «non peggio, non più di 5 punti meglio»: K 0,25 +0,3 · 0,5 +0,2 · 0,75 +0,1 · 1 −0,1. Adottato il primo dentro la banda,
   **0,25**. ⚠ Il criterio quasi non distingue (4 decimi da un capo all'altro): i due numeri pesano poco sul consenso, perché
   in coabitazione l'economia della porta è già in ripresa e il logorio è una frazione piccola. */
let COABITAZIONE_ECO_K = 0.25;       // let: la sweep D19 (L101-1) lo varia sul banco
let COABITAZIONE_LOGORIO_K = 0.25;   // let: idem
...
/* L104-2 · D-b È RITIRATA: la regola «si entra solo a una tappa» esisteva perché la presidenziale ricalcolava
   l'Assemblea. Con le urne separate la coabitazione si deriva a OGNI cambio di seggi o di Presidente: tappa,
   scioglimento, presidenziale, avvio della porta, salita al governo. */
function aggiornaCoabitazione(){
  if(typeof S==='undefined' || !S) return;
  ...
  if(ora){
    ...
    if(!era){
      ...
      S.coabitazione=true;
      S.sostegno=null;   // L101-1: l'appoggio esterno era al governo del Presidente, che non c'è più: rompendosi più tardi rimetteva S.minoranza a vero (misurato: 2 cadute in coabitazione)
...
/* ================================================================================================================
   L80-5 · IL GOVERNO CADE, NON IL PRESIDENTE — la sfiducia in un sistema semipresidenziale.

   PERCHE. Fino a oggi la mozione di censura chiamava `election()` per tutti, e in Francia questo mandava alle urne
   il PRESIDENTE — che nel gioco vince col testa a testa, quindi rigiocava la sua elezione. Misurato in L80-2:
   13,5 elezioni in dieci anni, la porta piu' rotta del presente. Nella V Repubblica la censura fa cadere il PRIMO
   MINISTRO: il Presidente resta, ne nomina un altro, e semmai scioglie l'Assemblea — ma quello e' un ATTO SUO, non
   una conseguenza. E' esattamente cio' che distingue quel sistema da un parlamentare, e nel gioco non c'era.

   ⚑ `PAESE.sistema` esisteva gia' su tutti e sedici i paesi e NON LO LEGGEVA NESSUNO: un campo dichiarato e morto,
   come il `mln` della valuta prima di L75-3. Da oggi e' vivo, e la Francia e' l'unico `semipresidenziale`.

   LA CERIMONIA riusa il rimpasto, non ne inventa una parallela (regola del 26/8): due ministri cambiano padrone,
   le correnti si irritano, la stampa punisce. ⚠ Con una differenza che la ricognizione ha trovato: `partnerRimpasto()`
   cerca un alleato che porti il blocco SOPRA 50, e in Francia RN+LR fa 44 — quindi torna sempre `null`. Il caso
   «nessun partner» non e' l'eccezione, e' la norma: si va avanti da soli, con un governo nuovo e gli stessi numeri.
   Il prezzo si paga comunque, ed e' il punto: cadere costa, ma non finisce la partita.

   NESSUN TETTO ai governi caduti, per ora: prima si misura la dispersione (regola della soglia-dopo-la-dispersione).
   ================================================================================================================ */
...
function governoCade(){
  ...
  if(p){
    /* c'e' un partner che porta i numeri: e' il rimpasto, con la sua cerimonia gia' scritta */
    if(typeof rimpastoEntra==='function') rimpastoEntra();
  } else {
    /* nessun partner: governo nuovo con gli stessi numeri — due ministeri cambiano mano lo stesso */
    var passati=0;
  ...
  /* L80-7 - LA PENA STA SULLA LINEA, NON SUL COLPO. Il -4/-2 qui sotto resta, ma come ECO: si vede in scheda
     il mese della caduta e poi la molla lo riassorbe (misurato in L80-6: chi cade quindici volte finiva come chi
     cadeva quattro). La pena vera va su `S.logorioAcc`, che la molla NON ce l'ha — e' la linea di L77-3. Ogni
     caduta vi aggiunge l'equivalente di CADUTA_LOGORIO_K mesi di logorio AL CONSENSO CORRENTE, cioe' esattamente
     la stessa forma del logorio mensile: chi e' popolare paga meno anche qui, ed e' voluto. */
  S.logorioAcc=(S.logorioAcc||0) + (typeof rateLogorioMese==='function' ? rateLogorioMese()*CADUTA_LOGORIO_K : 0);
  stampad(-4);
...
function sostegnoRompi(perche){
  ...
  S.sostegno=null; S.minoranza=!S.coabitazione && bloccoSeggi()<50;   // L101-1: in coabitazione la minoranza resta spenta
...
/* ================================================================================================================
   L104-2 · LE URNE DEL SEMIPRESIDENZIALE SI SEPARANO. Fino a qui la presidenziale e lo scioglimento erano la stessa
   `election()`: la presidenziale ricalcolava l'Assemblea dalle forze, e lo scioglimento rifaceva la presidenza (vinto,
   passava da `nextMandate` e il settennato ripartiva da capo). La storia non fa così, e il costo era misurato:
   minoranze «dall'urna» di 14-69 mesi in fr1960-fr1990 (L104-1). Ora, solo con `PAESE.sistema==='semipresidenziale'`:
     · la PRESIDENZIALE elegge e basta: `S.seggi` non si tocca (avviaAttesa, avviaNotte, nextMandate, la salita), e
       chi vince da sfidante eredita l'Assemblea in vigore;
     · lo SCIOGLIMENTO è un'urna di soli seggi (`urnaLegislativa()`): la notte dei seggi, la trattativa, e alla fine
       `chiudiLegislativa()` — niente `nextMandate`, il mandato presidenziale continua. Ammesso anche in coabitazione;
       i dodici mesi di `scioglimentoMesiMin` si contano dall'Assemblea in vigore (`S.legislaturaDa`).
   Parlamentare e presidenziale puro non passano da nessuna di queste righe. */
function sistemaSemipres(){ return !!PAESE && PAESE.sistema==='semipresidenziale'; }
...
function chiudiLegislativa(){
  ...
  initTenuta(); S.bloccoAtteso=bloccoQuota(); S.ultimoSondaggio=null; S.sondStorico=[];
  if(typeof bioFatto==='function'){ try{ bioFatto(T(S.coabitazione?'Le legislative anticipate danno l\'Assemblea agli altri: comincia una coabitazione.':'Le legislative anticipate si chiudono: il mandato all\'Eliseo continua.')); }catch(e){} }
  S.log.unshift({t:T('Legislative anticipate'), x:T(S.coabitazione?'L\'Assemblea nuova è degli altri: il mandato all\'Eliseo continua, in coabitazione.':(S.minoranza?'L\'Assemblea nuova non ha una maggioranza tua: il mandato all\'Eliseo continua, in minoranza.':'L\'Assemblea nuova è tua: il mandato all\'Eliseo continua.'))});
  document.getElementById('ov').classList.remove('on');
  genAgenda(false); generaTitolo(); render(); commitSnap();
}
...
/* L60-2 · gli snodi del decennio inglese '80. Falkland aprile-maggio '82 (il pilastro-cronaca è giugno, e ha `cond`);
   il secondo nodo il mese dopo la flotta. Minatori: gennaio-marzo '84, il braccio di ferro da giugno '84 a febbraio '85.
   Poll tax: da settembre '88 a giugno '89. La scissione dell'81 (Labour) e i quattro che bussano (Liberal): gennaio-marzo
   '81, ANCHE all'opposizione — è una carta di partito, e nel 1981 quei due partiti sono all'opposizione. */
function snodoFalklandDovuta(){     return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.falkland==null && S.year===1982 && S.month>=4 && S.month<=5; }
function snodoFalklandDueDovuta(){  return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.falkland==='flotta'; }
function snodoMinatori80Dovuta(){   return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.minatori80==null && S.year===1984 && S.month<=3; }
function snodoMinatori80DueDovuta(){return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.minatori80==='scorte' && ((S.year===1984 && S.month>=6) || (S.year===1985 && S.month<=2)); }
function snodoPolltaxDovuta(){      return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.polltax==null && ((S.year===1988 && S.month>=9) || (S.year===1989 && S.month<=6)); }
function snodoAlleanza81Dovuta(){   return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && S.alleanza==null && (S.partito==='uk_lab' || S.partito==='uk_lib') && S.year===1981 && S.month<=3; }
/* L60-2 · la scissione dell'81 col roster a cinque: n punti passano dal Labour a `uk_lib`, che diventa l'Alleanza
   (rinomina di roster, come la tappa per chi non gioca nessuno dei due). Se `nomeMio` è dato, è il giocatore
   liberale che accetta i quattro: il suo partito cambia nome per SUA scelta (il paletto L40-1 vale per le direttive). */
function alleanza81(n, nomeMio){
  if(typeof S==='undefined' || !S || !S.forze) return;
  var da='uk_lab', a='uk_lib';
  var q=Math.min(n, Math.max(0,(S.forze[da]||0)-2));
  S.forze[da]=(S.forze[da]||0)-q; S.forze[a]=(S.forze[a]||0)+q;
  if(S.forzePrev){ S.forzePrev[da]=S.forze[da]; S.forzePrev[a]=S.forze[a]; }
  S.rosterDelta = S.rosterDelta || {entra:[], esce:[], rinomina:[]};
  if(S.partito!==a || nomeMio) S.rosterDelta.rinomina.push({ id:a, nome:nomeMio||'Alleanza SDP-Liberali' });
  if(typeof applicaRosterDelta==='function') applicaRosterDelta(true);
  if(typeof rinormalizzaForze==='function') rinormalizzaForze();
}
/* L75-1 · gli snodi del decennio inglese '90. S1 la sfida al leader: novembre 1990, al governo, SOLO con anzianità ≥ 2 mandati
   (G11: chi non la vive riceve il pilastro). S2 il mercoledì nero: settembre 1992, al governo. S3 l'anima del partito:
   1994-95, all'opposizione, solo chi gioca il Labour. S4 la devoluzione: luglio-settembre 1997, al governo. */
function snodoCaduta90Dovuta(){    return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.caduta90==null && (S.mandatesWon||0)>=2 && S.year===1990 && S.month>=10 && S.month<=11; }
function snodoMercolediDovuta(){   return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.mercoledi==null && S.year===1992 && S.month>=8 && S.month<=9; }
function snodoAnimaDovuta(){       return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && S.opposizione && S.partito==='uk_lab' && S.anima==null && S.year>=1994 && S.year<=1995; }
function snodoDevoluzioneDovuta(){ return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.devoluzione==null && S.year===1997 && S.month>=7 && S.month<=9; }
/* L77-1 · gli snodi del decennio inglese 2000. Iraq: settembre 2002 - marzo 2003, al governo. Northern Rock:
   settembre-dicembre 2007, al governo. La coalizione: maggio-giugno 2010, al governo e SOLO con un parlamento appeso
   (parlamentoAppeso() di L61-5) — se il giocatore ha la maggioranza, la storia non lo riguarda. L'austerita': dal
   giugno 2010 al 2011, al governo e solo dopo che la coalizione e' stata decisa (o l'appeso non c'e' stato). */
function snodoIraqDovuta(){        return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.iraq==null && ((S.year===2002 && S.month>=9) || (S.year===2003 && S.month<=3)); }
function snodoRockDovuta(){        return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.rock==null && S.year===2007 && S.month>=9; }
function snodoCoalizione2010Dovuta(){ return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.coalizione2010==null && S.year===2010 && S.month>=5 && S.month<=6 && (typeof parlamentoAppeso!=='function' || parlamentoAppeso()); }
function snodoAusteritaDovuta(){   return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && !S.opposizione && S.austerita2010==null && ((S.year===2010 && S.month>=6) || S.year===2011); }
...
/* L77-4 - i gemelli dall'OPPOSIZIONE. Stessa finestra dello snodo di governo, condizione rovesciata, e la
   regola G1 nel gate: il flag del gemello dev'essere ancora null, cosi' chi lo ha giocato da premier non
   riceve anche la versione dall'aula. */
function snodoSuezOppDovuta(){    return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && S.opposizione && S.suezOpp==null && S.suez==null && S.year===1956 && S.month>=10; }
function snodoCoal2010OppDovuta(){ return typeof S!=='undefined' && S && S.era===LINEA_UK && S.livello===3 && S.opposizione && S.coal2010Opp==null && S.coalizione2010==null && S.year===2010 && S.month>=5 && S.month<=7 && (typeof parlamentoAppeso!=='function' || parlamentoAppeso()); }
...
/* L101-2 · GLI SNODI DEL DECENNIO FRANCESE '80. Finestre di tre mesi dal mese della scheda; nessuno dei quattro mesi
   (settembre 1981, marzo 1983, giugno 1984, luglio 1986) è di un pilastro. Il lato lo decide eliseoDiSinistra() (L101-1):
   S1 e S2 al governo hanno due versioni sullo stesso id (le sceglie l'agenda); dall'aula esiste solo contro un Eliseo di
   sinistra, perché il testo parla delle sue nazionalizzazioni. S3 è solo dell'Eliseo di sinistra al governo. S4 ha due
   versioni sullo stesso id: le ordinanze in coabitazione, le privatizzazioni al governo di destra senza coabitazione;
   il Presidente di sinistra che nell'86 tiene l'Assemblea non riceve S4 (dichiarato in data.js). */
function snodoRiforme81Dovuta(){     return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && !S.opposizione && S.riforme81==null && S.year===1981 && S.month>=9 && S.month<=11; }
function snodoRiforme81OppDovuta(){  return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && S.opposizione && S.riforme81Opp==null && S.riforme81==null && eliseoDiSinistra() && S.year===1981 && S.month>=9 && S.month<=11; }
function snodoRigore83Dovuta(){      return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && !S.opposizione && S.rigore83==null && S.year===1983 && S.month>=3 && S.month<=5; }
function snodoRigore83OppDovuta(){   return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && S.opposizione && S.rigore83Opp==null && S.rigore83==null && S.year===1983 && S.month>=3 && S.month<=5; }
function snodoScuola84Dovuta(){      return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && !S.opposizione && S.scuola84==null && eliseoDiSinistra() && S.year===1984 && S.month>=6 && S.month<=8; }
function snodoOrdinanze86Dovuta(){   return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && !S.opposizione && S.ordinanze86==null && (!!S.coabitazione || !eliseoDiSinistra()) && S.year===1986 && S.month>=7 && S.month<=9; }
/* LA CORSA AL FRANCO DELL'81 (§I-B): un'ancora sola, ottobre 1981 → gennaio 1982, nel gate parametrizzato e nello stesso
   registro del '50 (S.francoAncore): l'id a81 non collide con le ancore del '50. Esce dopo S1 se S1 ha preso il mese. */
const FRANCO80_ANCORE = [ {id:'a81', da:1981, daMese:10, a:1982, aMese:1} ];
function franco81AncoraDovuta(){
  if(typeof S==='undefined' || !S) return null;
  return ancoraValutaDovuta(LINEA_FR, FRANCO80_ANCORE, S.francoAncore, false);
}
/* L103-2 · GLI SNODI DEL DECENNIO FRANCESE '90. Finestre di tre mesi, nessun mese occupato da un pilastro. S1 e S2 sono del
   Presidente in tutti i rami (anche in coabitazione: il referendum e la difesa restano suoi); S3 e S4 solo col Presidente
   di destra senza coabitazione. S3 dall'aula è di chi gioca un partito di sinistra all'opposizione di un Presidente di
   destra. S4 si chiude a maggio: l'urna di soli seggi (L104-2) deve venire prima della tappa del 1997/6. */
function snodoRef92Dovuta(){        return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && !S.opposizione && S.ref92==null && S.year===1992 && S.month>=9 && S.month<=11; }
function snodoRef92OppDovuta(){     return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && S.opposizione && S.ref92Opp==null && S.ref92==null && S.year===1992 && S.month>=9 && S.month<=11; }
function snodoSarajevo94Dovuta(){   return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && !S.opposizione && S.sarajevo94==null && S.year===1994 && S.month>=2 && S.month<=4; }
function snodoPiano95Dovuta(){      return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && !S.opposizione && S.piano95==null && !eliseoDiSinistra() && !S.coabitazione && ((S.year===1995 && S.month>=11) || (S.year===1996 && S.month===1)); }
function snodoPiano95OppDovuta(){   var p=(typeof part==='function')?part(S&&S.partito):null;
  return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && S.opposizione && S.piano95Opp==null && !eliseoDiSinistra() && !!p && (p.asse||0)<0 && ((S.year===1995 && S.month>=11) || (S.year===1996 && S.month===1)); }
function snodoScioglimento97Dovuta(){ return typeof S!=='undefined' && S && S.era===LINEA_FR && S.livello===3 && !S.opposizione && S.scioglimento97==null && !eliseoDiSinistra() && !S.coabitazione && S.year===1997 && S.month>=4 && S.month<=5; }
/* LA CORSA AL FRANCO DEL '92 E DEL '93 (§I-B): due ancore, stesso registro S.francoAncore (a92, a93: nessuna collisione con
   a51/a55/a57/a81). ⚠ Il gate non guarda la coabitazione: la corsa del '93 arriva anche al Presidente in coabitazione. */
const FRANCO90_ANCORE = [ {id:'a92', da:1992, daMese:10, a:1993, aMese:1}, {id:'a93', da:1993, daMese:8, a:1993, aMese:12} ];
function franco90AncoraDovuta(){
  if(typeof S==='undefined' || !S) return null;
...
function startAttivista(){
  ...
  installaProfilo(GOVERNI_PROFILI['centro']); S.snap=Object.assign({},S.pol);   // nazionale neutro/dormiente (L104-3: dal punto unico) (come startLocale)
...
function diventaMinistro(){
  ...
  S.premier=generaPremier();
  installaProfilo(GOVERNI_PROFILI[profByAsse(S.premier.asse)]); S.snap=Object.assign({},S.pol);   // L104-3
  ...
  S.ministers=[]; S.coalizione=[S.partito]; S.seggi=(PAESE.coalizione||PAESE.comeSiVince==='parlamentare')?calcSeggi():null;
  S.ind.consenso=computeConsenso(); S.ind.deficit=computeDeficit(); S.ind.fiducia=targetFiducia();
  bioFatto('Da '+ruoloPrima+' a '+ruoloDicastero()+': la chiamata '+aCapitale()+'.');
  S.log.unshift({t:luoghiSub(T('La chiamata %ACAPITALE')),x:T('Il partito ti porta nel governo come %R. La scalata continua.').replace('%R',T(ruoloDicastero()))});
  if(typeof PROMO_FIORE!=='undefined') PROMO_FIORE=true;
...
/* L80-4 - «questo territorio e' del mio blocco?». Passa da staColBlocco quando si sa CHI chiede (il terzo
   argomento): cosi' un'area governata da un alleato dichiarato conta come tua anche sulla mappa, e non solo
   in parlamento. Senza il terzo argomento resta l'asse, che e' il comportamento di sempre. */
function compatibile(partitoId, asse, idTuo){
  if(idTuo && typeof staColBlocco==='function') return partitoId===idTuo || staColBlocco(partitoId, idTuo);
  const p=part(partitoId); return p ? Math.abs(p.asse-asse)<=1 : false;
}
...
/* L80-4 - a chi passa un territorio che cambia mano: se si sa chi e' il giocatore si usa la stessa nozione
   di blocco delle alleanze, altrimenti l'asse come prima. */
function scegliPartito(tuoLato, lean, asseTuo, idTuo){ const dentro=function(p){ return (idTuo && typeof staColBlocco==='function') ? (p.id===idTuo || staColBlocco(p.id, idTuo)) : Math.abs(p.asse-asseTuo)<=1; };
  const cand=PAESE.partiti.filter(function(p){ return tuoLato ? dentro(p) : !dentro(p); }); const pool=cand.length?cand:PAESE.partiti; let best=pool[0], bd=Infinity; for(const p of pool){ const d=Math.abs(p.asse-lean); if(d<bd){ bd=d; best=p; } } return best.id; }
...
function quotaTerritori(){ if(!S.territori || !S.territori.length) return null; const a=mioPartito().asse; return S.territori.filter(function(t){ return compatibile(t.partito,a,S.partito); }).length / S.territori.length * 100; }
/* ============================================================
...
/* ================================================================================================================
   ...
   e i testi li chiamano con un segnaposto, risolto alla resa come gia' fanno %TERR (arcoTerrSub) e %POT (potSub):
     %CAPITALE  -> «Roma» / «Londra»       %ACAPITALE -> «a Roma» / «ad Abuja»       %SEDE -> «Palazzo Chigi» / «Downing Street»
   La d eufonica solo davanti alla stessa vocale, come vuole l'uso moderno: «a Ottawa», ma «ad Abuja».
...
function decidiTerritori(ris){
  ...
  PAESE.territori.forEach(function(TE,i){
    ...
    const localShare=50 + wave + (TE.lean*aB*4) + ((dopo[i].spinta||0)*0.5) + (Math.random()*6-3);   // L64-2: la spinta della campagna PRIMA dell'onda
...
/* ================================================================================================================
   L73-2 · IL FINALE IN TRE VOCI — cosa hai fatto, cosa ne dice il paese, con che nome ti ricorderanno (DESIGN-FINALI).
   Tutto viene dallo STATO, mai da un testo fisso per esito: due carriere con dati diversi finiscono diverse.
   STORIA: le divergenze dalla storia (SNODI_STORICI, solo snodi, max 5) · le promesse (S.promesseEsito) · le leggi
   con la durata (S.leggiStorico) · la crisi che ti ha segnato (S.biografia.fatti) · e l'inventario di prima.
   VERDETTO: chi ti ha abbandonato (S.groups0 → S.groups) · chi ti ha succeduto (un nome: S.sfida.volto,
   S.governoAvversarioVolto, o trovaVolto()) · le correnti · il paese in una riga (S.ind0 → S.ind).
   TITOLO: l'epitaffio = ciò che hai costruito + ciò che hai pagato, con i dati dentro gli slot (il paletto:
   tuple-dati diverse → titoli diversi, misurato in .claude/misura-finali.js).
   ================================================================================================================ */
function divergenzeStoria(){
  if(typeof SNODI_STORICI==='undefined') return {div:[], seg:[]};
  const div=[], seg=[];
  for(const f in SNODI_STORICI){ const v=S[f]; if(v==null) continue; const D=SNODI_STORICI[f];
    if(D.storico.indexOf(v)>=0) seg.push(T(D.conforme));
...
function voltoSuccessore(reason){
  ...
  if(S.opposizione && S.governoAvversarioVolto) return {v:S.governoAvversarioVolto, tipo:'governo', p:(part(S.governoAvversario)||{}).nome||''};
  if(reason==='voto' && S.governoAvversarioVolto) return {v:S.governoAvversarioVolto, tipo:'governo', p:(part(S.governoAvversario)||{}).nome||''};
  const tv=(typeof trovaVolto==='function')?trovaVolto():null;
  if(tv && tv.volto) return {v:tv.volto, tipo:'dopo'};
  return null;
}
function generaFinale(reason){
  const B=S.biografia||bioVuota(), storia=[], verdetto=[];
  const anno=S.year;
  /* A1 · le divergenze */
  const dz=divergenzeStoria();
  if(dz.div.length) storia.push(dz.div.slice(0,5).join(' ') + (dz.seg.length?' '+T('Hai fatto quello che fece la storia %L.').replace('%L', dz.seg.slice(0,3).join(', ')):''));
  else if(dz.seg.length) storia.push(T('Hai attraversato la storia senza cambiarla — che è una scelta anche quella.')+' '+T('Hai fatto quello che fece la storia %L.').replace('%L', dz.seg.slice(0,3).join(', ')));
  /* A2 · le promesse */
  const pe=S.promesseEsito||[];
  if(pe.length){ const ok=pe.filter(function(p){return p.mantenuta;}).length, ko=pe.find(function(p){return !p.mantenuta;});
    if(pe.length===1) storia.push(T(ok?'Avevi promesso una cosa, a %G, e l\'hai mantenuta.':'Avevi promesso una cosa, a %G, e non l\'hai mantenuta: non l\'hanno dimenticato.').replace('%G',nomeGruppoEp(pe[0].grp)));
    else if(!ko) storia.push(T('Avevi promesso %M cose, e le hai mantenute tutte.').replace('%M',pe.length));
    else storia.push(T('Avevi promesso %M cose. Ne hai mantenute %N; la promessa a %G non l\'hanno dimenticata.').replace('%M',pe.length).replace('%N',ok).replace('%G',nomeGruppoEp(ko.grp))); }
  /* A3 · le leggi, con la durata di quelle abrogate */
  if(B.leggiFirmate.length) storia.push(T('Portano la tua firma: ')+B.leggiFirmate.slice(0,5).map(function(n){return T(n);}).join(', ')+(B.leggiFirmate.length>5?' '+T('e altre'):'')+'.');
  (S.leggiStorico||[]).filter(function(l){return l.abrogataAnno!=null;}).slice(0,2).forEach(function(l){ const d=l.abrogataAnno-l.anno; storia.push(d>=1?T('La legge «%L» che avevi voluto è durata %N anni.').replace('%L',T(l.nome)).replace('%N',d):T('La legge «%L» che avevi voluto è durata meno di un anno.').replace('%L',T(l.nome))); });
  /* A4 · la crisi che ti ha segnato: la prima crisi del debito, o la prima emergenza */
  const fatti=(B.fatti||[]);
  const fc=fatti.find(function(f){return /crisi del debito/i.test(f.testo||'');}), fe=fatti.find(function(f){return /emergenza/i.test(f.testo||'');});
  if(fc) storia.push(T('La crisi del debito del %Y è quella che ti ha segnato.').replace('%Y',fc.anno)); else if(fe) storia.push(T('L\'emergenza del %Y è quella che ti ha segnato.').replace('%Y',fe.anno));
  /* A5 · L80-5 — i governi caduti. Slot nuovo, e porta il DATO (il paletto dell'epitaffio: le tuple-dati diverse
  ...
  if(gp && gp.v0!=null){ const nome=nomeGruppoEp(gp.id), a=Math.round(gp.v0), b=Math.round(gp.v);
    if(b<a-8){ let r=T('%G ti aveva dato il %A: te ne lascia il %B.').replace('%G',nome).replace('%A',a).replace('%B',b); if(reason==='rivolta' && S.rivoltaGruppo===gp.id) r+=' '+T('Sono loro che ti hanno mandato a casa.'); verdetto.push(r); }
    else verdetto.push(T('%G ti aveva dato il %A e te ne lascia il %B: nessuno ti ha abbandonato davvero.').replace('%G',nome).replace('%A',a).replace('%B',b)); }
  /* B2 · chi ti ha succeduto */
  const vs=voltoSuccessore(reason);
  if(vs){ if(vs.tipo==='partito') verdetto.push(T('Il partito è passato a %V, che aveva aspettato il suo momento.').replace('%V',vs.v)); else if(vs.tipo==='governo') verdetto.push(T('Al governo è andato %V, del %P.').replace('%V',vs.v).replace('%P',T(vs.p||''))); else verdetto.push(T('Dopo di te, il partito ha scelto %V.').replace('%V',vs.v)); }
  /* B3 · le correnti */
  if(S.correnti && S.correnti.length>=2){ const cs=S.correnti.slice().sort(function(a,b){return b.umore-a.umore;}); const nm=function(c){ const D=CORRENTI_DEF.find(function(d){return d.id===c.id;})||{}; return T(D.nome||c.id); };
    if(cs[cs.length-1].umore<45) verdetto.push(T('%A ti hanno seguito fino all\'ultimo; %B ti avevano mollato.').replace('%A',nm(cs[0])).replace('%B',nm(cs[cs.length-1]))); else verdetto.push(T('Le correnti ti hanno seguito fino all\'ultimo, tutte.')); }
  /* B4 · il paese in una riga */
  if(S.ind0){ const dd=S.ind.debt-S.ind0.debt, du=S.ind.unemp-S.ind0.unemp;
    verdetto.push(T('Lasci un paese %D e %L di quello che avevi trovato.').replace('%D',T(dd>3?'più indebitato':dd<-3?'meno indebitato':'col debito dov\'era')).replace('%L',T(du<-0.7?'con più lavoro':du>0.7?'con meno lavoro':'con lo stesso lavoro'))); }
  /* C · l'epitaffio: costruito + pagato, coi dati negli slot */
  const nLeggi=B.leggiFirmate.length, nDiv=dz.div.length, promOk=pe.length&&pe.every(function(p){return p.mantenuta;});
  let costr;
  if(nDiv>=2) costr=T('Chi cambiò la storia in %N punti').replace('%N',nDiv);
  else if(nDiv===1) costr=T('Chi cambiò la storia in un punto');
  else if(nLeggi>=3) costr=T('Il legislatore delle %N leggi').replace('%N',nLeggi);
  else if(promOk && pe.length>=2) costr=T('Chi mantenne le sue %N promesse').replace('%N',pe.length);
  else if(S.ind0 && S.ind.debt<=S.ind0.debt-5) costr=T('Il custode dei conti (debito dal %A al %B)').replace('%A',Math.round(S.ind0.debt)).replace('%B',Math.round(S.ind.debt));
  else if(nLeggi>=1) costr=T('Chi firmò una legge sola');
  else costr=T('Chi attraversò la storia senza toccarla');
  const gN=gp?nomeGruppoEp(gp.id):'', gV=gp?Math.round(gp.v):'';
  let pag;
  if(reason==='rivolta') pag=T(', caduto nel %Y per la rivolta di %G').replace('%Y',anno).replace('%G',nomeGruppoEp(S.rivoltaGruppo||(gp&&gp.id)));
  else if(reason==='crisi') pag=T(', sfiduciato nel %Y con %G al %V').replace('%Y',anno).replace('%G',gN).replace('%V',gV);
  else if(reason==='insolvenza') pag=T(', travolto dai conti nel %Y').replace('%Y',anno);
  else if(reason==='congresso'||reason==='primaria') pag=T(', che perse il partito nel %Y (%V)').replace('%Y',anno).replace('%V',(vs&&vs.v)||T('lo sfidante'));
  else if(reason==='voto') pag=T(', battuto alle urne nel %Y da %V').replace('%Y',anno).replace('%V',(vs&&vs.v)||'');
...
function generaEpilogo(reason){
  ...
  const _F=(typeof generaFinale==='function')?generaFinale(reason):null;   // L73-2: la storia in tre voci viene prima dell'inventario
  ...
  if(_F){ _F.storia.forEach(function(p){ par.push(p); }); }
  else if(B.leggiFirmate.length) par.push(T('Portano la tua firma: ')+B.leggiFirmate.slice(0,5).map(function(n){return T(n);}).join(', ')+(B.leggiFirmate.length>5?' '+T('e altre'):'')+'.');
  const t2=[];
  ...
  else if(B.origine==='locale' && S.livello>1) par.push(luoghiSub(T('Dalla gavetta locale fino ai palazzi di %CAPITALE: la strada è stata lunga.')));
...
function trovaVolto(){
  ...
  if(S.territori && PAESE.territori && part(S.partito)){
    ...
    PAESE.territori.forEach(function(TE,i){
      const t=S.territori[i]; if(!t || !compatibile(t.partito,a,S.partito)) return;
...
function aggiornaSfida(){
  ...
  if(S.sfida){
    ...
    /* L64-3 — promosso: per 12 mesi non matura; poi, se l'umore resta basso, torna con +2 (una volta) */
    if(S.sfida.promossoMese!=null){ if(mese-S.sfida.promossoMese<12) return; if(!S.sfida.rafforzato && umoreMedio()<45){ S.sfida.rafforzato=true; S.sfida.maturazione=(S.sfida.maturazione||0)+2; S.log.unshift({t:T('Lo sfidante'), x:T('%V torna più forte: l\'incarico non è bastato, e adesso ha anche un curriculum.').replace('%V',S.sfida.volto)}); } }
    if(umoreMedio()<(S.sfida.isolato?40:45)) S.sfida.maturazione=(S.sfida.maturazione||0)+1;   // L64-3: isolato → matura solo sotto 40
    return;
  }
  if(S.primariaUltima!=null && mese-S.primariaUltima<12) return;   // raffreddamento dopo una primaria vinta
  /* L64-4 — chi hai messo in lista ti sfida: se la sua corrente scende sotto 40 (non 35), la sfida nasce col SUO volto */
  const seme=S.sfidaSeme;
  const arrS=(seme && S.correnti.find(function(c){return c.id===seme.corrente && c.umore<40;}))||null;
  const arr=arrS||S.correnti.find(function(c){return c.umore<35;});
  if(!arr) return;
  const v=(arrS && seme.volto)?{ volto:seme.volto, carica:T('capolista'), area:null, fonte:'liste', simbolo:false }:trovaVolto(); if(!v) return;
  if(arrS) S.sfidaSeme=null;   // il seme si consuma quando germoglia
  const D=CORRENTI_DEF.find(function(d){return d.id===arr.id;})||{};
...
/* ================================================================================================================
   L64-3 · LO SFIDANTE SI PUÒ GESTIRE (DESIGN-SUPERFICI-VIVE §B). `S.sfida` ha già volto, corrente, maturazione; oggi
   l'unica risposta era indiretta (alzare l'umore finché «la sfida rientra»). Tre azioni sulla scheda-partito, gemelle
   di azioneIncarico: PROMUOVI (maturazione a 0, la sua corrente +6, entra nella rosa dei ministri con competenza alta;
   se fra 12 mesi l'umore medio è ancora sotto 45 torna con +2 di maturazione — «e adesso ha anche un curriculum»),
   ISOLA (la sua corrente −8, le altre +3, maturazione −1 e da lì matura solo sotto 40), AFFRONTA SUBITO (primaria
   anticipata con +3 perché scegli tu il momento — e si può perdere). Promuovi e isola sono mosse di partito (stesso
   raffreddamento di azioneIncarico); ciascuna una volta per sfida. Misurato in .claude/misura-sfida.js.
   ================================================================================================================ */
function sfidaPromuovi(){
  if(!S.sfida || S.sfida.promossoMese!=null || !mossaPartitoDisponibile()) return;
  const mese=S.year*12+S.month; S.mossaPartito=mese;
  S.sfida.promossoMese=mese; S.sfida.maturazione=0;
  S.correnti.forEach(function(c){ c.umore=clamp(c.umore+(c.id===S.sfida.corrente?6:-2),0,100); });
  const D=CORRENTI_DEF.find(function(d){return d.id===S.sfida.corrente;})||{};
  S.candidatoPromosso={ nm:S.sfida.volto, profile:(D.id==='pontieri'?'tecnico':D.id==='militanti'?'progressista':'conservatore'), comp:3 };
  S.log.unshift({t:T('Lo sfidante'), x:T('Hai promosso %V: un incarico di peso, e la sua corrente si placa. Se fra un anno l\'umore è ancora basso, tornerà più forte.').replace('%V',S.sfida.volto)});
  if(typeof render==='function') render();
}
function sfidaIsola(){
  if(!S.sfida || S.sfida.isolato || !mossaPartitoDisponibile()) return;
  const mese=S.year*12+S.month; S.mossaPartito=mese;
  S.sfida.isolato=true; S.sfida.maturazione=Math.max(0,(S.sfida.maturazione||0)-1);
  S.correnti.forEach(function(c){ c.umore=clamp(c.umore+(c.id===S.sfida.corrente?-8:3),0,100); });
  S.log.unshift({t:T('Lo sfidante'), x:T('Hai isolato %V: la sua corrente si stringe attorno a lui, le altre respirano. La sfida rallenta, non sparisce.').replace('%V',S.sfida.volto)});
  if(typeof render==='function') render();
}
function sfidaAffronta(){
  if(!S.sfida) return;
...
function genAgendaRamo(first){
  ...
  if(S.opposizione){   // all'opposizione: intermedia (se c'è) oppure la tua carta + (a volte) un evento del governo AI
    ...
    if(!first && typeof snodoAlleanza81Dovuta==='function' && snodoAlleanza81Dovuta()){ S.agenda.push({kind:'event', data:(S.partito==='uk_lib'?QUATTRO_EV:SCISSIONE81_EV), resolved:false}); agendaSolo(); return; }   // L60-2: la scissione dell'81 arriva anche all'opposizione
    ...
    if(!first && typeof snodoRiforme81OppDovuta==='function' && snodoRiforme81OppDovuta()){ S.agenda.push({kind:'event', data:RIFORME81_OPP_EV, resolved:false}); agendaSolo(); return; }   // L101-2: le riforme dell'81, dall'aula
  ...
  /* L77-1 · gli snodi del 2000 */
  if(!first && typeof snodoIraqDovuta==='function' && snodoIraqDovuta()){ S.agenda.push({kind:'event', data:IRAQ_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoRockDovuta==='function' && snodoRockDovuta()){ S.agenda.push({kind:'event', data:ROCK_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoCoalizione2010Dovuta==='function' && snodoCoalizione2010Dovuta()){ S.agenda.push({kind:'event', data:COALIZIONE2010_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoAusteritaDovuta==='function' && snodoAusteritaDovuta()){ S.agenda.push({kind:'event', data:AUSTERITA_EV, resolved:false}); agendaSolo(); return; }
  /* L75-1 · gli snodi del '90 */
  if(!first && typeof snodoCaduta90Dovuta==='function' && snodoCaduta90Dovuta()){ S.agenda.push({kind:'event', data:CADUTA90_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoMercolediDovuta==='function' && snodoMercolediDovuta()){ S.agenda.push({kind:'event', data:MERCOLEDI_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoDevoluzioneDovuta==='function' && snodoDevoluzioneDovuta()){ S.agenda.push({kind:'event', data:DEVOLUZIONE_EV, resolved:false}); agendaSolo(); return; }
  /* L60-2 · gli snodi dell'80: prima i secondi nodi (conseguenze di scelte già fatte), poi i primi. */
  if(!first && typeof snodoFalklandDueDovuta==='function' && snodoFalklandDueDovuta()){ S.agenda.push({kind:'event', data:FALKLAND_DUE_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoFalklandDovuta==='function' && snodoFalklandDovuta()){ S.agenda.push({kind:'event', data:FALKLAND_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoMinatori80DueDovuta==='function' && snodoMinatori80DueDovuta()){ S.agenda.push({kind:'event', data:MINATORI80_DUE_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoMinatori80Dovuta==='function' && snodoMinatori80Dovuta()){ S.agenda.push({kind:'event', data:MINATORI80_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoPolltaxDovuta==='function' && snodoPolltaxDovuta()){ S.agenda.push({kind:'event', data:POLLTAX_EV, resolved:false}); agendaSolo(); return; }
  if(!first && typeof snodoAlleanza81Dovuta==='function' && snodoAlleanza81Dovuta()){ S.agenda.push({kind:'event', data:(S.partito==='uk_lib'?QUATTRO_EV:SCISSIONE81_EV), resolved:false}); agendaSolo(); return; }
  ...
  if(!first && typeof snodoRiforme81Dovuta==='function' && snodoRiforme81Dovuta()){ S.agenda.push({kind:'event', data:(eliseoDiSinistra() ? RIFORME81_EV : RIFORME81_DX_EV), resolved:false}); agendaSolo(); return; }   // L101-2
  if(!first && typeof snodoRigore83Dovuta==='function' && snodoRigore83Dovuta()){ S.agenda.push({kind:'event', data:(eliseoDiSinistra() ? RIGORE83_EV : RIGORE83_DX_EV), resolved:false}); agendaSolo(); return; }   // L101-2 (due versioni, residuo)
  if(!first && typeof snodoScuola84Dovuta==='function' && snodoScuola84Dovuta()){ S.agenda.push({kind:'event', data:SCUOLA84_EV, resolved:false}); agendaSolo(); return; }        // L101-2
  if(!first && typeof snodoOrdinanze86Dovuta==='function' && snodoOrdinanze86Dovuta()){ S.agenda.push({kind:'event', data:(S.coabitazione ? ORDINANZE86_EV : PRIVATIZZAZIONI86_EV), resolved:false}); agendaSolo(); return; }   // L101-2
  if(!first && typeof snodoRef92Dovuta==='function' && snodoRef92Dovuta()){ S.agenda.push({kind:'event', data:REF92_EV, resolved:false}); agendaSolo(); return; }        // L103-2
...
/* ================================================================================================================
   L67-2 · IL RIEPILOGO DEGLI EFFETTI — «le conseguenze non si vedono».
   La riga `e:` di una scelta e qualitativa; il numero non compariva mai. Qui, attorno alla risoluzione di una
   carta, si fotografa CIO CHE IL GIOCATORE HA NEL CRUSCOTTO (gruppi, correnti, indicatori, valute d'opposizione)
   prima e dopo, e si conserva in `it.esiti` la lista delle variazioni sopra soglia: dato puro, serializzabile.
   Nessun numero interno (niente uMod, ciclo, target): solo grandezze che una scheda mostra gia.
   LA SOGLIA E MISURATA (2/9, 3.580 decisioni su 15 carriere): il 63% delle decisioni muove qualcosa, e fra 0,1
   e 1 punto la quota NON cambia — non c'e un pendio, c'e un gradino. Quindi 1 punto per gruppi/correnti/
   indicatori; il debito si muove per decimi (mediana 0,4) e ha la sua soglia a 0,2. Media attesa: ~1,6 voci.
   Massimo 5 voci, le piu grandi prima. Effetti differiti: il motore non ha una coda, quindi nessuna riga al futuro.
   ================================================================================================================ */
const ESITI_MAX=5;
function fotoEsiti(){
  const f={};
  if(!S) return f;
  if(S.ind){ ['consenso','debt','stampa','fiducia','reputazione'].forEach(function(k){ if(S.ind[k]!=null) f['ind:'+k]=S.ind[k]; }); }
  if(S.groups) GROUPS.forEach(function(g){ if(S.groups[g.id]!=null) f['grp:'+g.id]=S.groups[g.id]; });
  if(S.correnti) S.correnti.forEach(function(c){ if(c&&c.umore!=null) f['cor:'+c.id]=c.umore; });
  if(S.opposizione){ if(S.visibilita!=null) f['opp:visibilita']=S.visibilita; if(S.credibilita!=null) f['opp:credibilita']=S.credibilita; }
  return f;
}
function diffEsiti(prima,dopo){
  const out=[];
  Object.keys(dopo).forEach(function(k){
    if(prima[k]==null) return;
    const d=dopo[k]-prima[k], soglia=(k==='ind:debt')?0.2:1;
    if(Math.abs(d)+1e-9<soglia) return;
    out.push({k:k, d:Math.round(d*10)/10});
  });
  out.sort(function(a,b){ return Math.abs(b.d)-Math.abs(a.d); });
  return out.slice(0,ESITI_MAX);
}
...
function resaPromesseCampagna(){
  ...
  vecchie.forEach(function(p){
    ...
    const mantenuta=!((p.colpi||0)>=6);
    S.promesseEsito=S.promesseEsito||[]; S.promesseEsito.push({grp:p.grp, anno:S.year, mantenuta:mantenuta});   // L73-2: il finale tira le somme
    if(!mantenuta){ gd(p.grp,-5); stampad(-4); S.log.unshift({t:T('La promessa tradita'), x:T('In campagna avevi promesso attenzione a %G, poi i fatti: ora te la rinfacciano.').replace('%G',nome)}); }
...
function pickCampagnaNazionale(){
  ...
  campSforzo();   // L64-2: apre lo sforzo della stagione (una volta) e lo annuncia nel log
  if(S.campNaz.n===0) resaPromesseCampagna();          // la resa apre la stagione (le forze si muovono, i sondaggi la prezzano)
  /* L64-4 — il primo mese di stagione è LE LISTE: obbligata (prende il posto del beat, da sola) e senza ritorno (una per stagione) */
  if(!S.campNaz.liste && S.correnti && typeof LISTE_EV!=='undefined'){ S.campNaz.liste=true; S.campNazUltimo=mese; S.campNaz.n++; return {kind:'event', data:LISTE_EV, resolved:false}; }
...
function forseSondaggio(){
  ...
  const valR=Math.round(clamp(v.val + (Math.random()*2-1)*m + ((typeof biasSondaggio==='function')?biasSondaggio():0), 0, 100));   // rumore uniforme ±m, clampato · L75-1: + il bias d'epoca
...
/* ================================================================================================================
   L64-2 · LA CAMPAGNA SI GIOCA SULLA MAPPA (DESIGN-SUPERFICI-VIVE §C). Nei sei mesi di stagione hai uno SFORZO FINITO
   (`S.campNaz.sforzo`, dato puro) da distribuire sui territori dal pannello dell'area: ogni punto speso qui è un
   punto non speso là, e a fine stagione quel che resta SI PERDE — è la decisione a scadenza che le schede non avevano.
   Il costo dipende dall'area (roccaforte 1, in bilico 1, terreno avversario 2) e il POTERE LOCALE lo abbassa (una rete
   che esiste rende la campagna più efficace); il ritorno è la SPINTA del territorio (`S.territori[i].spinta`), che il
   voto legge PRIMA dell'onda nazionale (decidiTerritori) — il verso è finalmente dal basso in alto — e che alla vigilia
   confluisce nelle forze del blocco (applicaSpintaForze, prima di calcSeggi). Le roccaforti rendono poco (ci voti già),
   le aree in bilico molto, il terreno avversario costa il doppio e paga in reputazione anche se perdi.
   Nessun obbligo: chi ignora la campagna gioca lo stesso, peggio. Chiude L59-5 per costruzione.
   ================================================================================================================ */
const CAMP_SFORZO_BASE=6;
function inCampagna(){ if(typeof S==='undefined'||!S||S.livello!==3||!PAESE.territori||!S.territori) return false; const fine=(PAESE.mandatoMesi||60), mm=meseMandato(); return mm>=fine-6 && mm<fine; }
function mesiAllaFine(){ const fine=(PAESE.mandatoMesi||60); return Math.max(0, fine-meseMandato()); }
function sforzoIniziale(){ const pl=(S.potereLocale!=null)?S.potereLocale:50; return CAMP_SFORZO_BASE + (pl>=60?1:0); }
function campSforzo(){ if(!inCampagna()) return 0; if(!S.campNaz) S.campNaz={piazza:0,promMantenuta:false,n:0}; if(S.campNaz.sforzo==null){ S.campNaz.sforzo=sforzoIniziale(); S.campNaz.sforzo0=S.campNaz.sforzo; S.campNaz.speso={}; S.log.unshift({t:T('Campagna sul territorio'), x:T('La campagna è aperta: hai %N punti di sforzo da spendere sulla mappa entro %M mesi. Quelli non spesi si perdono.').replace('%N',S.campNaz.sforzo).replace('%M',mesiAllaFine())}); } return S.campNaz.sforzo; }
function tipoTerritorio(i){ const TE=PAESE.territori[i], t=S.territori[i]||{}; const tuo=compatibile(t.partito, mioPartito().asse, S.partito); const L=Math.abs(TE.lean); if(L>=2) return tuo?'roccaforte':'avversario'; return tuo?'bilico':'bilico'; }
function costoInvestimento(i){ const tipo=tipoTerritorio(i); let c=(tipo==='avversario')?2:1; if(tipo==='avversario' && (S.potereLocale||0)>=60) c=1; return c; }
...
function investiTerritorio(i){
  ...
  S.decisioniScadenza=(S.decisioniScadenza||0)+1;   // la misura del cantiere: decisioni a scadenza originate da una scheda
  const TE=PAESE.territori[i];
  S.log.unshift({t:T('Campagna sul territorio'), x:T('Sforzo di campagna su %T: %C punti spesi, ne restano %R.').replace('%T',nomeTerr(TE)).replace('%C',c).replace('%R',S.campNaz.sforzo)});
  if(typeof render==='function') render(); if(typeof commitSnap==='function') commitSnap();
}
/* Alla vigilia: la spinta confluisce nelle forze del blocco (il punto di giunzione con i seggi: avviaAttesa → calcSeggi).
...
function chiudiSforzo(){ if(S.campNaz && S.campNaz.sforzo>0) S.log.unshift({t:T('Campagna sul territorio'), x:T('Fine della campagna: %N punti di sforzo non spesi, persi.').replace('%N',S.campNaz.sforzo)}); }
function avviaAttesa(){
  let sistema, vero;
  applicaSpintaForze();   // L64-2: la campagna sul territorio confluisce nelle forze PRIMA che i seggi si calcolino
  if(PAESE.comeSiVince==='parlamentare' || urnaLegislativa()){   // L104-2: lo scioglimento nel semipresidenziale è un'urna di soli seggi
    ...
    if((PAESE.coalizione||PAESE.comeSiVince==='parlamentare') && !sistemaSemipres()) S.seggi=calcSeggi();   // L104-2: la presidenziale non rifà l'Assemblea
...
function renderAttesa(){
  ...
  const truffaNota = (snodoTruffaAttivo() && S.leggeTruffa==='approvata')
    ...
    : '';
  document.getElementById('modal').innerHTML=`<div class="mt"><div class="kicker">${T(PAESE.nome)} · ${T(urnaLegislativa()?'Legislative anticipate':(S.elezioniAnticipate?'Elezioni anticipate':'Elezioni'))} ${S.year}</div><h2>${T('La chiusura della campagna')}</h2></div>
...
function avviaNotte(){
  ...
  if(ATTESA){ sistema=ATTESA.sistema; vero=ATTESA.vero; ATTESA=null; }   // usa il VERO congelato nell'attesa (non ricalcola: la chiusura non lo tocca)
    ...
    if(PAESE.comeSiVince==='parlamentare' || urnaLegislativa()){ S.seggi=calcSeggi(); sistema='parlamentare'; vero={ seggi:Object.assign({},S.seggi) }; }
    else { if((PAESE.coalizione||PAESE.comeSiVince==='parlamentare') && !sistemaSemipres()) S.seggi=calcSeggi(); sistema='candidato'; vero=testaATesta(); }   // L104-2
  }
...
function renderTrattativa(){
  ...
  if(reached) action=`<button class="opt" style="border-color:var(--acc);background:var(--acc-bg)" onclick="${confirmFn}"><span class="ol" style="color:var(--acc-ink)">${T(voto?'Conferma la maggioranza':'Forma il governo')} →</span><span class="oe">${total} ${T('seggi')}${cb?` +${cb} ${T('credibilità')} → ${Math.round(eff)}`:' '+T('su 100')}</span></button>`;
      ...
      : (coabitazioneInVista() ? `<button class="opt" style="border-color:var(--acc)" onclick="${confirmFn}"><span class="ol">${T('Coabitazione')} →</span><span class="oe">${T('solo %N seggi · l\'Assemblea è degli altri: il Primo ministro sarà del loro blocco').replace('%N',total)}</span></button>`
  ...
  const kicker = voto?`${T(urnaLegislativa()?'Legislative anticipate':(S.elezioniAnticipate?'Elezioni anticipate':'Elezioni'))} ${S.year}` : T(ctx==='rinnovo'?'Nuovo mandato':'Formazione del governo');
...
/* L104-2 · con i membri che la trattativa ha in mano, l'Assemblea darebbe una coabitazione? (solo semipresidenziale) */
function coabitazioneInVista(){
  if(!sistemaSemipres() || !COAL || S.opposizione) return false;
...
/* L104-2 · l'esito delle legislative anticipate nel semipresidenziale: la tabella dei seggi di esitoSeggi, ma nessun
   mandato nuovo e nessun passaggio all'opposizione — il Presidente resta; cambia solo di chi è l'Assemblea. */
function esitoLegislativa(win, total){
  const seggi=S.seggi, ps=[...PAESE.partiti].sort((a,b)=>(seggi[b.id]||0)-(seggi[a.id]||0));
  const rows=ps.map(p=>{ const gov=S.coalizione.includes(p.id);
    return `<div style="padding:6px 0">
      <div style="display:flex;justify-content:space-between;font-size:13px"><span style="font-weight:${gov?700:500}">${T(p.nome)} ${gov?'<span class="chip" style="background:var(--acc-bg);color:var(--acc-ink)">'+T('Governo')+'</span>':''}</span><span class="mono">${seggi[p.id]||0}</span></div>
      <div class="bar"><i style="width:${clamp(seggi[p.id]||0,2,100)}%;background:${gov?'var(--acc)':'var(--mut2)'}"></i></div></div>`; }).join('');
  const sub= win ? T('La tua coalizione ottiene <b>%N seggi</b> su 100: l\'Assemblea nuova è tua, e il mandato all\'Eliseo continua.').replace('%N',total)
                : T('La tua coalizione si ferma a <b>%N seggi</b>: l\'Assemblea nuova non è tua. Il mandato all\'Eliseo continua, con una maggioranza da cercare o in coabitazione.').replace('%N',total);
  document.getElementById('modal').innerHTML=`<div class="mt"><div class="kicker">${T('Legislative anticipate')} · ${S.year}</div><h2>${titoloEsito(win)}</h2></div>
    <div class="mtext">${sub}</div>
    <div style="padding:0 18px 6px">${rows}</div>
    <div class="choices"><button class="opt" style="border-color:${win?'var(--acc)':'var(--neg)'}" onclick="chiudiLegislativa()"><span class="ol">${T('Torna all\'Eliseo →')}</span></button></div>`;
  document.getElementById('ov').classList.add('on');
}
...
function nextMandate(){
  ...
  if(typeof aggiornaCoabitazione==='function') aggiornaCoabitazione();   // L100-2/L104-2: alla presidenziale la coabitazione si deriva dall'Assemblea in vigore (si entra, si resta, si esce)
  ...
  chiudiSforzo(); decidiTerritoriNazionale();   // L64-2: lo sforzo non speso si perde; le aree lavorate si decidono dal basso
...
function entraOpposizione(w){
  ...
  installaProfilo(GOVERNI_PROFILI[prof]); S.snap=Object.assign({},S.pol); S.leggiSnap=Object.assign({},S.leggi);   // L104-3: senza salto di disavanzo · eredita le politiche dell'avversario, contabilità RP pulita
...
function setLegge(id){
  ...
  S.leggiStorico=S.leggiStorico||[];   // L73-2: {id, nome, anno, abrogataAnno} — il finale dice quanto è durata
  if(!cur) S.leggiStorico.push({id:L.id, nome:L.nome, anno:S.year}); else { const _r=S.leggiStorico.slice().reverse().find(function(x){return x.id===L.id && x.abrogataAnno==null;}); if(_r) _r.abrogataAnno=S.year; }
...
/* L72-1 — la rivolta: il testo di chiusura dipende dal gruppo che ti ha voltato le spalle (S.rivoltaGruppo). */
const RIVOLTA_TESTI={
  lavoratori:  'Sciopero generale a oltranza: le fabbriche si fermano e i sindacati non trattano più. Il governo cade davanti alla piazza.',
  pensionati:  'Le piazze si riempiono di anziani, e sono metà del paese: nessun alleato regge. Il governo cade.',
...
function applySnap(snap){
  ...
  if(S.decisioniScadenza===undefined){ S.decisioniScadenza=0; S.visite={mappa:0, partito:0}; }   // L64-2
  if(S.gruppo===undefined){ S.gruppo=null; S.sfidaSeme=null; S.candidatoPromosso=null; }   // L64-3/4
  if(S.scioglimentiChiesti===undefined){ S.scioglimentiChiesti=0; S.scioglimentoScelto=false; }   // L56-1
  if(S.legislaturaDa===undefined) S.legislaturaDa=null;   // L104-2
